...
public class AsignacionController {
    ...
    private final AsignacionN asignacionN = new AsignacionN();
    ...
    public void handle(HttpExchange ex) throws IOException {
        ...
        try {
            if ("GET".equals(met) && id == null) {
                List<Asignacion> lista = asignacionN.getAll();
                responder(ex, 200, ok(listaJson(lista), "Asignaciones obtenidas correctamente"));
            } else if ("GET".equals(met) && id != null) {
                Asignacion a = asignacionN.getById(id);
                if (a == null) responder(ex, 404, err("Asignacion no encontrada"));
                else           responder(ex, 200, ok(a.toJson(), "Asignacion obtenida correctamente"));
            } else if ("POST".equals(met)) {
                Asignacion a = asignacionN.create(parseBody(leerBody(ex)));
                responder(ex, 201, ok(a.toJson(), "Asignacion creada correctamente"));
            } else if ("PUT".equals(met) && id != null) {
                Asignacion a = asignacionN.update(id, parseBody(leerBody(ex)));
                if (a == null) responder(ex, 404, err("Asignacion no encontrada"));
                else           responder(ex, 200, ok(a.toJson(), "Asignacion actualizada correctamente"));
            } else if ("DELETE".equals(met) && id != null) {
                boolean ok = asignacionN.delete(id);
                if (!ok) responder(ex, 404, err("Asignacion no encontrada"));
                else     responder(ex, 200, ok("null", "Asignacion eliminada correctamente"));
            } else {
    ...
    private static String listaJson(List<Asignacion> lista) {
