...
public class ConvocatoriaController {
    ...
    private final ConvocatoriaN convocatoriaN = new ConvocatoriaN();
    ...
    public void handle(HttpExchange ex) throws IOException {
        ...
        try {
            if ("GET".equals(met) && id == null) {
                List<Convocatoria> lista = convocatoriaN.getAll();
                responder(ex, 200, ok(listaJson(lista), "Convocatorias obtenidas correctamente"));
            } else if ("GET".equals(met) && id != null) {
                Convocatoria conv = convocatoriaN.getById(id);
                if (conv == null) responder(ex, 404, err("Convocatoria no encontrada"));
                else              responder(ex, 200, ok(conv.toJson(), "Convocatoria obtenida correctamente"));
            } else if ("POST".equals(met)) {
                Convocatoria conv = convocatoriaN.create(parseBody(leerBody(ex)));
                responder(ex, 201, ok(conv.toJson(), "Convocatoria creada correctamente"));
            } else if ("PUT".equals(met) && id != null) {
                Convocatoria conv = convocatoriaN.update(id, parseBody(leerBody(ex)));
                if (conv == null) responder(ex, 404, err("Convocatoria no encontrada"));
                else              responder(ex, 200, ok(conv.toJson(), "Convocatoria actualizada correctamente"));
            } else if ("DELETE".equals(met) && id != null) {
                boolean ok = convocatoriaN.delete(id);
                if (!ok) responder(ex, 404, err("Convocatoria no encontrada"));
                else     responder(ex, 200, ok("null", "Convocatoria eliminada correctamente"));
            } else {
    ...
    private static String listaJson(List<Convocatoria> lista) {
